...
export const useUpdateCompanyInfo = () => {
    ...
    const updateConpanyInfo = async (name, description, location, email, phoneNumber) => {
        setIsLoading(true);
        setError(null);
        ...
        try {
            ...
            const response = await fetch(`http://localhost:4000/api/company/${companyId}`, {
                ...
                body: JSON.stringify({ name, description, location, phoneNumber, email}),
                credentials: 'include'
            });
    ...
    return { updateConpanyInfo, isLoading, error };
};